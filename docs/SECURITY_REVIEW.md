...
| Missing fees silently treated as zero | Critical operational | Fixed locally / per-action wallet estimates pending | A strict 15-item ledger counts known cost and worst-case item allocations, preserves the immutable 100 STRK ceiling, rejects floating point/omitted items/double-state records, and blocks an action until its fresh estimate fits its cap |
