...
| Missing fees silently treated as zero | Critical operational | Fixed locally / wallet estimates pending | A strict 15-item budget ledger counts actual plus projected cost, preserves the immutable 100 STRK ceiling, rejects floating point/omitted items/double-state records and makes the signing command fail until all estimates are explicit |
