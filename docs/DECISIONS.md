...
## 2026-09-02 — Resume secret workflows only through verified local imports

Reload-safe transaction metadata did not restore the researcher or vendor cryptographic state. Recovery package version 3 now includes the program public key and a domain-separated HMAC over every package field. Its strict, bounded importer authenticates the ciphertext without displaying plaintext and recomputes the program binding, case/report/ciphertext commitments and case signing-key pair. Separate importers recompute public-manifest commitments and prove that vendor public/private halves agree through X25519 key agreement. Cross-artifact mismatches fail closed. Files remain explicit, memory-only inputs: no backend, browser persistence or secret logging is introduced, and self-contained authentication is not represented as protection against package theft.
