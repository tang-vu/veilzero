...
The repository is reproducible for its implemented read-only app and Cairo build. GitHub Pages deployment run `33507533923` succeeded for the verified commit; the anonymous URL returned HTTP 200 and contained the destination-bound claim status. Release is not mainnet-complete: live Wallet API note-ID binding, deployment, live pool ABI/prover validation, three qualifying transactions and a real demo video remain explicitly blocked/deferred.
