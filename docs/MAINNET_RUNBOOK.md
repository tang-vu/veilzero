...
Claim gate: after the helper is deployed and the program, case and reward authorization exist, `prepareDestinationBoundClaim` first calls `wallet_strk20PrepareInvoke(..., true)`. The contract accepts its zero signature only at transaction version `2^128 + 3`; two contract-checked markers surround the resolved note ID. The client requires an empty preview proof and the verified pool target, signs that ID, calls `wallet_strk20PrepareInvoke(..., false)`, then rejects a changed pool, note drift or incomplete proof. Run the browser prepare-only harness with the chosen live wallet before enabling a separate claim signing gate. The harness discards the proof and never submits. Never log or persist either prepared proof.
...
Run `pnpm prepare:deployment` with process-scoped `STARKNET_RPC_URL` and `STRK20_POOL_ADDRESS` to verify artifact, pool, UDC and declaration readiness without an account. After obtaining only the dedicated browser wallet's public address, set `VEILZERO_DEPLOYER_ADDRESS` and repeat to add the expected unique UDC address and constructor pool binding. Both modes are read-only and deliberately do not estimate fees; obtain fresh wallet estimates before presenting either human signing gate.
