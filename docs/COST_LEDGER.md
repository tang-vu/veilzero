...
Current actual: **0 STRK**. Known projected pool fees for the three qualifying actions: **18 STRK**, based on the read-only mainnet observation at block `14205166` and reconfirmed for the reserve-custody release candidate at block `14208014`. Declaration, deployment, funding, vendor lifecycle calls and network fees remain unestimated; therefore the total projection is not yet gate-ready. Unknown is never treated as zero, and the live fee must be re-read immediately before each signature.
