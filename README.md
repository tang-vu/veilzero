...
| Machine-enforced mainnet budget | Implemented + unit/CI tested; per-action estimates pending | Exact decimal accounting covers reserve exposure, activation, declaration, deployment, admin calls and three pool actions; fixed item caps keep maximum exposure at 86 STRK and each signing gate requires that action's current estimate |
...
pnpm gate:budget -- declare-class-network # replace the item ID for each signing gate
